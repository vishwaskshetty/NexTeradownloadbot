...
async function runTests() {
  ...
  try {
    ...
    await db.adminAction.deleteMany({ where: { targetUserTelegramId: BigInt(testUserId) } });
    await db.user.deleteMany({ where: { telegramId: BigInt(testUserId) } });

    // 1. Create Test User (Initially FREE)
    console.log('1️⃣ Creating test user...');
    let user = await userService.getOrCreateUser(testUserId, 'test_premium_user', 'Test', 'User');
    ...
    const freeLimit = await usageService.getDailyLimitForUser(user.id);
    console.log(`   Free User Daily Limit: ${freeLimit}`);
    if (freeLimit !== 5) throw new Error('Expected free limit 5');

    // 2. Add Premium (30 days)
    console.log('\n2️⃣ Add Premium for user ID (30 Days)...');
    const addResult = await userService.addPremium(testAdminId, testUserId, 30);
    console.log(`   ✅ Added 30 days premium. New Expiry: ${addResult.newExpiry.toISOString()}`);
    
    // Test 9: Premium Daily Limit
    const premLimit = await usageService.getDailyLimitForUser(user.id);
    console.log(`   Premium User Daily Limit: ${premLimit}`);
    if (premLimit !== 50) throw new Error('Expected premium limit 50');

    // 3. Add Premium to existing active premium user (Extension test)
    console.log('\n3️⃣ Add Premium to existing active user (+30 Days Extension)...');
    const addAgainResult = await userService.addPremium(testAdminId, testUserId, 30);
    ...
    if (expectedDiffDays !== 30) throw new Error('Expected expiry to extend by exactly 30 days');

    // 4. Extend Premium (+15 days)
    console.log('\n4️⃣ Extend Premium (+15 Days)...');
    const extResult = await userService.extendPremium(testAdminId, testUserId, 15);
    console.log(`   ✅ Extended expiry to: ${extResult.newExpiry.toISOString()}`);

    // 5. View Premium User Details
    console.log('\n5️⃣ View Premium User Details...');
    const detail = await userService.getPremiumUserDetail(testUserId);
    console.log(`   Status: ${detail?.statusText}, Remaining: ${detail?.remainingDays} days, Daily Limit: ${detail?.dailyLimit}`);
    if (!detail?.isActive) throw new Error('Expected user to be active premium');
    ...
    const expiredDetail = await userService.getPremiumUserDetail(testUserId);
    ...
    try {
      await userService.addPremium(testAdminId, nonExistentUserId, 30);
    ...
    const isAdmin = adminService.isAdmin(testUserId);
    console.log(`   Is test user ${testUserId} admin? ${isAdmin}`);
    if (isAdmin) throw new Error('Test user should not be admin');

    // 9. Remove Premium
    console.log('\n9️⃣ Remove Premium...');
    await userService.addPremium(testAdminId, testUserId, 30); // Re-add
    const remResult = await userService.removePremium(testAdminId, testUserId);
    ...
    if (auditLogs.length < 4) throw new Error('Expected audit log entries for all premium actions');
