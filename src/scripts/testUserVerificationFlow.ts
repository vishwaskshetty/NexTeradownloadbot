...
async function main() {
  ...
  const user = await userService.getOrCreateUser(testTelegramId, 'arolinks_test_user', 'AroLinks', 'Test');
  ...
  const isVerifiedBefore = await verificationService.isUserVerified(user.id);
  console.log(`1️⃣ Initial Verification Status: ${isVerifiedBefore ? 'VERIFIED ❌' : 'UNVERIFIED ✅'}`);
  if (isVerifiedBefore) throw new Error('User should start unverified.');
  ...
  (config as any).VERIFICATION_BASE_URL = 'https://my-test-bot.onrender.com';
  (config as any).SHORTENER_API_KEY = process.env.SHORTENER_API_KEY || 'test_key';
  ...
  const testSession = await db.verificationSession.create({
    data: {
      userId: user.id,
