...
export class ForceSubService {
   ...
   */
  async getForceSubStatus(): Promise<boolean> {
    const { redis } = require('../redis');
    const cached = await redis.get('FORCE_SUB_ENABLED');
    if (cached !== null) return cached === 'true';

    const setting = await db.botSetting.findUnique({ where: { key: 'FORCE_SUB_ENABLED' } });
   ...
   */
  async setForceSubStatus(enabled: boolean): Promise<void> {
    ...
    logger.info(`Admin set FORCE_SUB_ENABLED to ${enabled}`);
  }

  /**
   * Retrieves list of required channels
   */
  async getRequiredChannels(): Promise<ForceSubChannel[]> {
    const setting = await db.botSetting.findUnique({ where: { key: 'FORCE_SUB_CHANNELS' } });
    if (!setting || !setting.value) return [];
    try {
      return JSON.parse(setting.value) as ForceSubChannel[];
    } catch {
      return [];
    }
  }
   ...
   */
  async addChannel(channel: ForceSubChannel): Promise<void> {
    ...
    logger.info(`Added force sub channel: ${channel.name} (${channel.id})`);
  }

  /**
   * Removes a required channel by ID or username
   */
  async removeChannel(channelIdOrUsername: string): Promise<boolean> {
    const channels = await this.getRequiredChannels();
    const filtered = channels.filter(c => 
      c.id !== channelIdOrUsername && 
    ...
    logger.info(`Removed force sub channel: ${channelIdOrUsername}`);
    return true;
  }
   ...
   */
  async getCustomMessage(): Promise<string> {
    const setting = await db.botSetting.findUnique({ where: { key: 'FORCE_SUB_MESSAGE' } });
   ...
   */
  async setCustomMessage(msg: string): Promise<void> {
    await db.botSetting.upsert({
   ...
   */
  async checkUserMembership(telegram: any, telegramUserId: number | bigint): Promise<{ isMember: boolean; missingChannels: ForceSubChannel[] }> {
    const channels = await this.getRequiredChannels();
    if (channels.length === 0) return { isMember: true, missingChannels: [] };

    const missingChannels: ForceSubChannel[] = [];
    ...
    for (const channel of channels) {
      try {
        ...
        logger.warn(`Could not verify channel membership for ${channel.name} (${channel.id}): ${err.message}`);
        missingChannels.push(channel);
      }
    }
...
export const forceSubService = new ForceSubService();
