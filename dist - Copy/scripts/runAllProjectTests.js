...
const token_service_1 = require("../verification/token.service");
async function runAllTests() {
    ...
    try {
        ...
        await AdminService_1.adminService.setReferralStatus(true);
        await AdminService_1.adminService.setReferralsRequired(10);
        await AdminService_1.adminService.setReferralRewardDays(5);
        // Clean up test data
        await db_1.db.adminAction.deleteMany({ where: { OR: [{ adminTelegramId: BigInt(adminTelegramId) }, { targetUserTelegramId: BigInt(user1TelegramId) }, { targetUserTelegramId: BigInt(user2TelegramId) }] } });
        await db_1.db.userUsage.deleteMany({});
        await db_1.db.referralMilestone.deleteMany({});
        await db_1.db.referral.deleteMany({});
        await db_1.db.verificationSession.deleteMany({});
        await db_1.db.user.deleteMany({ where: { OR: [{ telegramId: BigInt(user1TelegramId) }, { telegramId: BigInt(user2TelegramId) }] } });
        // Create Test Users
        const u1 = await UserService_1.userService.getOrCreateUser(user1TelegramId, 'user_one', 'User', 'One');
        const u2 = await UserService_1.userService.getOrCreateUser(user2TelegramId, 'user_two', 'User', 'Two');
        console.log('--------------------------------------------------');
        ...
        const flowResult = await verification_service_1.verificationService.createVerificationFlow(u1.id, 'NexTeraDownloadBot');
        ...
        const valResult = await verification_service_1.verificationService.validateToken(flowResult.token);
        ...
        const reusedVal = await verification_service_1.verificationService.validateToken(flowResult.token);
        console.log(`    Reused Token Result: ${reusedVal.message}`);
        if (reusedVal.success)
            throw new Error('Reused token should have been rejected');
        // Test 1.4: Validate Invalid Token Rejection
        console.log('1.4 Validating Invalid Token Rejection...');
        const invalidVal = await verification_service_1.verificationService.validateToken('invalid_token_1234567890');
        console.log(`    Invalid Token Result: ${invalidVal.message}`);
        if (invalidVal.success)
        ...
        const expiredRawToken = token_service_1.tokenService.generateSecureToken(32);
        const expiredHash = token_service_1.tokenService.hashToken(expiredRawToken);
        await db_1.db.verificationSession.create({
            data: {
                userId: u1.id,
        ...
        const expiredVal = await verification_service_1.verificationService.validateToken(expiredRawToken);
        console.log(`    Expired Token Result: ${expiredVal.message}`);
        if (expiredVal.success)
        ...
        const initialUsage = await UsageService_1.usageService.getUsage(u1.id);
        await UsageService_1.usageService.recordFailedRequest(u1.id);
        const postFailedUsage = await UsageService_1.usageService.getUsage(u1.id);
        ...
        const selfRef = await ReferralService_1.referralService.processReferral(user1TelegramId, user1TelegramId);
        console.log(`    Self-referral result: ${selfRef.message}`);
        if (selfRef.success)
            throw new Error('Self-referral must be rejected');
        // Test 3.2: Valid Referral Registration
        console.log('3.2 Testing Valid Referral Registration...');
        const validRef = await ReferralService_1.referralService.processReferral(user1TelegramId, user2TelegramId);
        console.log(`    Valid referral result: ${validRef.message}`);
        if (!validRef.success)
            throw new Error('Valid referral failed');
        // Test 3.3: Duplicate Referral Rejection
        console.log('3.3 Testing Duplicate Referral Rejection...');
        const dupRef = await ReferralService_1.referralService.processReferral(user1TelegramId, user2TelegramId);
        console.log(`    Duplicate referral result: ${dupRef.message}`);
        if (dupRef.success)
            throw new Error('Duplicate referral must be rejected');
        ...
        for (let i = 3; i <= 10; i++) {
            ...
            await UserService_1.userService.getOrCreateUser(dummyId, `dummy_${i}`);
            await ReferralService_1.referralService.processReferral(user1TelegramId, dummyId);
        }
        // Now total referrals = 9
        let stats = await ReferralService_1.referralService.getReferralStats(user1TelegramId);
        console.log(`    Referrals Count: ${stats.totalReferrals}/10, Progress Bar: ${stats.progressBar}`);
        if (stats.totalReferrals !== 9)
            throw new Error('Expected 9 referrals');
        let u1Check = await UserService_1.userService.getUser(user1TelegramId);
        if (u1Check?.isPremium)
            throw new Error('User should not be premium at 9 referrals');
        // Add 10th referral -> Triggers Milestone #1 (+5 days premium)
        console.log('3.5 Adding 10th Referral (Triggers Milestone #1)...');
        await UserService_1.userService.getOrCreateUser(300000011, 'dummy_11');
        await ReferralService_1.referralService.processReferral(user1TelegramId, 300000011);
        stats = await ReferralService_1.referralService.getReferralStats(user1TelegramId);
        ...
        u1Check = await UserService_1.userService.getUser(user1TelegramId);
        console.log(`    User #1 Premium Status: isPremium=${u1Check?.isPremium}, Expiry=${u1Check?.premiumExpiresAt?.toISOString()}`);
        if (!u1Check?.isPremium)
            throw new Error('Expected user to be awarded premium at 10 referrals');
        ...
        await ReferralService_1.referralService.checkAndAwardMilestones(u1Check.id, BigInt(user1TelegramId));
        const milestonesCount = await db_1.db.referralMilestone.count({ where: { referrerUserId: u1Check.id } });
        console.log(`    Milestones recorded in DB: ${milestonesCount}`);
        if (milestonesCount !== 1)
            throw new Error('Milestone should not be duplicated');
        console.log('\n--------------------------------------------------');
        ...
        const extResult = await UserService_1.userService.extendPremium(adminTelegramId, user1TelegramId, 30);
        ...
        if (diffDays !== 30)
            throw new Error('Manual extension must extend existing expiry by exactly 30 days');
        ...
        const downgradedUser = await UserService_1.userService.getUser(user1TelegramId);
        ...
        // Cleanup test data
        await db_1.db.adminAction.deleteMany({ where: { targetUserTelegramId: BigInt(user1TelegramId) } });
        await db_1.db.referralMilestone.deleteMany({});
        await db_1.db.referral.deleteMany({});
        await db_1.db.verificationSession.deleteMany({});
        await db_1.db.userUsage.deleteMany({});
        await db_1.db.user.deleteMany({ where: { telegramId: { in: [BigInt(user1TelegramId), BigInt(user2TelegramId)] } } });
