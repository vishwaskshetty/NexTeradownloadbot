...
class UserService {
     ...
     */
    async checkAndUpdatePremiumStatus(user) {
        if (user.plan === 'PREMIUM' || user.isPremium) {
            if (user.premiumExpiresAt && user.premiumExpiresAt <= new Date()) {
                logger_1.logger.info(`[UserService] Premium expired for user ${user.telegramId}. Downgrading to FREE.`);
    ...
    async getOrCreateUser(telegramId, username, firstName, lastName, languageCode) {
        ...
        if (!user) {
            ...
            logger_1.logger.info({ telegramId: telegramId.toString() }, 'Created new user');
        }
    ...
    async getUsers(page = 1, limit = 10) {
        ...
        let users = await db_1.db.user.findMany({
            skip,
            take: limit,
        ...
        users = await Promise.all(users.map(u => this.checkAndUpdatePremiumStatus(u)));
     ...
     */
    async addPremium(adminTelegramId, targetTelegramId, days) {
        let user = await db_1.db.user.findUnique({
        ...
        // Audit Log
        await db_1.db.adminAction.create({
            data: {
                adminUserId: null,
        ...
        logger_1.logger.info(`[UserService] Admin ${adminTelegramId} added ${days} days premium to ${targetTelegramId}`);
        return {
            user: updatedUser,
            previousExpiry,
            newExpiry,
     ...
     */
    async extendPremium(adminTelegramId, targetTelegramId, days) {
        let user = await db_1.db.user.findUnique({
            where: { telegramId: BigInt(targetTelegramId) }
        });
        if (!user) {
        ...
        logger_1.logger.info(`[UserService] Admin ${adminTelegramId} extended premium for ${targetTelegramId} by ${days} days`);
        return {
            user: updatedUser,
            previousExpiry,
            newExpiry,
     ...
     */
    async removePremium(adminTelegramId, targetTelegramId) {
        const user = await db_1.db.user.findUnique({
            where: { telegramId: BigInt(targetTelegramId) }
        });
        if (!user) {
        ...
        logger_1.logger.info(`[UserService] Admin ${adminTelegramId} removed premium for ${targetTelegramId}`);
        return {
            user: updatedUser,
            previousExpiry
        };
     ...
     */
    async getPremiumUserDetail(targetTelegramId) {
        const rawUser = await db_1.db.user.findUnique({
            where: { telegramId: BigInt(targetTelegramId) },
            include: { usage: true }
        });
        if (!rawUser)
        ...
        return {
            ...
            remainingDays,
            dailyLimit: isActive ? config_1.config.PREMIUM_DAILY_LIMIT : config_1.config.FREE_DAILY_LIMIT
        };
    }
    /**
     * LIST PREMIUM USERS WITH PAGINATION AND SEARCH BY TELEGRAM USER ID / USERNAME
     */
    async getPremiumUsersList(page = 1, limit = 10, search) {
        const skip = Math.max(0, (page - 1) * limit);
        let whereClause = {
            OR: [
                { isPremium: true },
