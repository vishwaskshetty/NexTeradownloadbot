...
exports.runUsageAccountingTests = runUsageAccountingTests;
const UserService_1 = require("../services/UserService");
const JobService_1 = require("../services/JobService");
const UsageService_1 = require("../services/UsageService");
const db_1 = require("../db");
async function runUsageAccountingTests() {
    const log = [];
    let total = 0;
    let passed = 0;
    let failed = 0;
    ...
    const user = await UserService_1.userService.getOrCreateUser(testTelegramId, 'usage_test_user', 'Usage', 'Test');
    // Reset test user's usage
    await db_1.db.userUsage.upsert({
    ...
    const getUsageCount = async () => (await UsageService_1.usageService.getUsage(user.id)).dailyRequests;
    // --------------------------------------------------
    // Test 1: Link submitted → usage unchanged
    // --------------------------------------------------
    total++;
    const initialUsage = await getUsageCount();
    const jobResult1 = await JobService_1.jobService.createJob(user.id, 'https://terabox.com/s/test_1', 'TeraBox');
    ...
    // --------------------------------------------------
    total++;
    await JobService_1.jobService.updateJobStatus(jobResult1.job.id, 'QUEUED');
    ...
    // --------------------------------------------------
    total++;
    await JobService_1.jobService.updateJobStatus(jobResult1.job.id, 'PROCESSING');
    ...
    // --------------------------------------------------
    total++;
    await JobService_1.jobService.failJob(jobResult1.job.id, 'Provider resolution failed');
    ...
    // --------------------------------------------------
    total++;
    const jobResult2 = await JobService_1.jobService.createJob(user.id, 'https://diskwala.com/file/test_2', 'Diskwala');
    await JobService_1.jobService.updateJobStatus(jobResult2.job.id, 'PROCESSING');
    await JobService_1.jobService.failJob(jobResult2.job.id, 'Network stream interrupted');
    await UsageService_1.usageService.recordFailedRequest(user.id);
    ...
    // --------------------------------------------------
    total++;
    const jobResult3 = await JobService_1.jobService.createJob(user.id, 'https://diskwala.com/file/test_3', 'Diskwala');
    await JobService_1.jobService.updateJobStatus(jobResult3.job.id, 'FILE_READY');
    await JobService_1.jobService.updateJobStatus(jobResult3.job.id, 'SENDING');
    await JobService_1.jobService.failJob(jobResult3.job.id, 'Telegram sendDocument failed');
    ...
    // --------------------------------------------------
    total++;
    const jobResult4 = await JobService_1.jobService.createJob(user.id, 'https://diskwala.com/file/test_4', 'Diskwala');
    await JobService_1.jobService.cancelJob(jobResult4.job.id);
    ...
    // --------------------------------------------------
    total++;
    const jobResult5 = await JobService_1.jobService.createJob(user.id, 'https://diskwala.com/file/test_5', 'Diskwala');
    await JobService_1.jobService.updateJobStatus(jobResult5.job.id, 'PROCESSING');
    await JobService_1.jobService.updateJobStatus(jobResult5.job.id, 'SENDING');
    const compResult1 = await JobService_1.jobService.completeJob(jobResult5.job.id, 1024n);
    ...
    // --------------------------------------------------
    total++;
    const compResult2 = await JobService_1.jobService.completeJob(jobResult5.job.id, 1024n);
    ...
    // --------------------------------------------------
    total++;
    const jobResult6 = await JobService_1.jobService.createJob(user.id, 'https://diskwala.com/file/test_6', 'Diskwala');
    await Promise.all([
        JobService_1.jobService.completeJob(jobResult6.job.id, 2048n),
        JobService_1.jobService.completeJob(jobResult6.job.id, 2048n)
    ]);
    ...
    // --------------------------------------------------
    total++;
    await db_1.db.user.update({ where: { id: user.id }, data: { plan: 'PREMIUM' } });
    const premJob = await JobService_1.jobService.createJob(user.id, 'https://diskwala.com/file/prem_test', 'Diskwala');
    await JobService_1.jobService.failJob(premJob.job.id, 'Simulated failure for premium');
    const usage11Failed = await getUsageCount();
    await JobService_1.jobService.completeJob(premJob.job.id, 4096n);
    ...
    // --------------------------------------------------
    total++;
    const dailyLimit = await UsageService_1.usageService.getDailyLimit('PREMIUM');
    ...
    // Cleanup test user
    try {
        await db_1.db.job.deleteMany({ where: { userId: user.id } });
