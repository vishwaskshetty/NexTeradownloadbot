...
exports.runCallbackAudit = runCallbackAudit;
 ...
 */
async function runCallbackAudit() {
    const log = [];
    let total = 0;
    let passed = 0;
    let failed = 0;
    ...
    const adminUser = await UserService_1.userService.getOrCreateUser(6059191947, 'test_admin', 'Admin', 'User');
    const normalUser = await UserService_1.userService.getOrCreateUser(999999999, 'test_user', 'Normal', 'User');
    log.push('==================================================');
    ...
    for (const item of callbacksToTest) {
        ...
        try {
            await (0, callback_1.callbackHandler)(mockCtx);
